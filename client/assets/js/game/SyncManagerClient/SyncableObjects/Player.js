var PlayerType;
(function (PlayerType) {
    PlayerType[PlayerType["remote"] = 0] = "remote";
    PlayerType[PlayerType["local"] = 1] = "local";
...
class Player extends SyncableObject {
    constructor(playerData) {
        ...
        if (playerData.socketID === SyncManagerClient.socket.id) {
            if (Player.localPlayer)
                throw new Error('local player allready connected');
    ...
    dragSegment(segement, nextSegment) {
